...
src/
  ...
  goose/actions.ts   它会做什么：动作的关键帧数据（只放数据）
...
- 每条动作轨道：首帧 frame = 0，末帧 frame = duration。默认可循环动作还要求首值 = 末值；`loopable: false` 的动作跳过该项，且不得在分镜里 `loops > 1`。`engine.assertLoopable` 在模块加载时强制检查，不满足直接抛错。
